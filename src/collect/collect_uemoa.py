...
def collecter_donnees():
    ...
    logging.basicConfig(
        filename="meteo_collecte.log",
        level=logging.INFO,
        format="%(asctime)s - %(levelname)s - %(message)s"
    )

    # === Connexion PostgreSQL (optionnel)
    db = 'entrepot_uemoa'
    user = 'postgres'
    ...
    toutes_donnees = []
    total_villes = sum(len(villes) for pays, villes in villes_uemoa.items())
    compteur = 0
    ...
    for pays, villes in villes_uemoa.items():
        for ville_info in villes:
            ...
            lat = ville_info["lat"]
            lon = ville_info["lon"]
            print(f"[{compteur}/{total_villes}] 🔄 {ville}, {pays}")
            try:
                ...
                if response.status_code == 200:
                    ...
                    if "daily" in data and "time" in data["daily"]:
                        ...
                        df["Ville"] = ville
                        df["Pays"] = pays
                        df["datecollecte"] = date_collecte
                        toutes_donnees.append(df)
                        logging.info(f"✅ {ville}, {pays}")
                    else:
                        logging.warning(f"Structure JSON invalide pour {ville}, {pays}")
                elif response.status_code == 429:
                    logging.warning(f"429 trop de requêtes {ville}")
                else:
                    logging.error(f"Erreur {ville} : {response.status_code}")
            except Exception as e:
                logging.exception(f"Exception {ville} : {e}")
            time.sleep(1.5)

    if not toutes_donnees:
        logging.error("Aucune donnée")
        sys.exit(1)

    df_final = pd.concat(toutes_donnees, ignore_index=True)
    df_final.to_csv("historique_meteo_uemoa.csv", index=False)
    logging.info("✅ Fichier historique_meteo_uemoa.csv enregistré")
